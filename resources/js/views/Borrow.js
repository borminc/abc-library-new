...
const Borrow = (props) => {
	...
	// useEffect(() => {
	// 	axios.get('/api/borrow?book_id=' + bookId).then(res => {
	// 		setBorrow(res.data);
	// 	})
	// }, [bookId]);
	...
	function handleSubmit(event) {
		...
		axios.post('/api/borrow?book_id=' + bookId)
		.then(res => {
			setBorrow(res.data);
		})
	}
	...
	const Modal = (book) => {
		return(
		<div className="modal fade" id={'myModal' + book.id} tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
			<div className="modal-dialog">
				<div className="modal-content">
					<div className="modal-body">
						<p>Are you sure want to borrow <span className="text-success"> {book.title} </span>?</p>
					</div>
					<div className="modal-footer">
						<button type="button" className="btn btn-secondary btn-sm" data-bs-dismiss="modal">Close</button>
						<form onSubmit={handleSubmit}>
							<button type="submit" className="btn btn-primary btn-sm">Borrow</button>
						</form>
					</div>
				</div>
			</div>
			...
			{book && (
				<div className='container pt-5 mt-5'>
									...
									<table className='table small'>
										...
										<tbody>
											...
											<tr>
												...
												{(book.stock == 0)? (<td className="text-danger">Out of Stock</td>) : <td>{book.stock}</td>}
											</tr>
										</tbody>
									</table>
								</div>
					...
					<div className='p-5'>
						...
						<button type='submit' className='btn btn-primary' data-bs-toggle="modal" data-bs-target={'#myModal' + book.id}>
							Borrow
						</button>
						{Modal(book)}
					</div>
